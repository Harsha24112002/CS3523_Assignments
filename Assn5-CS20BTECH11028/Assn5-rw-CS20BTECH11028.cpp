...
void* Writer(void* args)
{
	...
	for(int i=0;i<kw;i++)
	{
		...
		// Printing the request time here to print as thread safe
		...
		// To load the current ime into the exit time
...
void* Reader(void* args)
{
	...
	for(int i=0;i<kr;i++)
	{
		...
		sem_post(&mutex);
		// Releasing the lock once the reader count is incremented

		// Simulation of CS
		usleep(exp_dist1(generator)*MILLI);

		sem_wait(&mutex);
		reader_count--; // After the thread exits, decrementing the reader count
		struct timeval exit_time;
...
int main()
{
	...
	out.open("RW-log.txt");
	times.open("Average_time.txt", std::ios_base::in | std::ios_base::app);
	...
	if(times.is_open())
	{
		times << "-- Waiting times in RW -- "<< std::endl;
